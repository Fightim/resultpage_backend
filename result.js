...
app.get("/db", (req, res) => {
  ...
  console.log(token);
  axios
    .get("https://dev.cloud-gui.com/rds/config", {
...
app.post("/text", (req, res) => {
  ...
  axios
    ...
    .then(function (response) {
      ...
      connection.query(
        "CREATE DATABASE IF NOT EXISTS my_db CHARACTER SET UTF8; USE my_db; CREATE TABLE IF NOT EXISTS result(id INT UNSIGNED NOT NULL AUTO_INCREMENT, name VARCHAR(100) NULL, input_text TEXT NOT NULL, input_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL, endpoint TEXT NOT NULL, PRIMARY KEY(id)); INSERT INTO result (name, input_text, endpoint) VALUES (?, ?, ?);",
        [name, text, response.data.host],
        function (err, result) {
          if (err) {
            console.log(err);
            res.status(404).json({
            ...
            connection.query(
              "SELECT * FROM result WHERE id = (SELECT MAX(id) FROM result)",
